		...
		init : function(options, name) {
			...
			var dtGridObject = {
				...
				resetPager: function(){
					...
					var pager = dtGridReflectionObj.pager;
					pager.pageSize = dtGridReflectionObj.option.pageSize;
					pager.startRecord = 0;
					pager.nowPage = 1;
					pager.recordCount = 0;
					pager.pageCount = 0;
				...
				load : function(){
					//定义表格对象映像
					var dtGridReflectionObj = this;
					//解除事件传递状态 modify by nick
					dtGridReflectionObj.option.stopEvent=0;
					
					//===触发即将刷新视图事件 by nick===
					if(dtGridReflectionObj.option.onWillRefreshView) {
						dtGridReflectionObj.option.onWillRefreshView();
					}
					//================================
						...
						if(!dtGridReflectionObj.option.ajaxLoad){
							...
							if(dtGridReflectionObj.option.loadAll){
								...
								$.ajax({
									type:'post',
									url:url,
									data: {sortParameter:dtGridReflectionObj.option.sortParameter}, //null
									...
									success:function(datas){
										dtGridReflectionObj.hideProcessBar(function(){
											//处理原始数据集
											//===modify by nick===
											if (dtGridReflectionObj.option.didLoadedDataPreprocess) {
												var preResult = dtGridReflectionObj.option.didLoadedDataPreprocess('original', datas);
												if (typeof preResult=='boolean') //错误结果
													dtGridReflectionObj.originalDatas = {};
												else
													dtGridReflectionObj.originalDatas = preResult;
											} else {
												dtGridReflectionObj.originalDatas = json_parse(datas);//;$.parseJSON(datas);
											}
											//===modify by nick end===
					...
					if(!dtGridReflectionObj.option.ajaxLoad||dtGridReflectionObj.option.loadAll){
						...
						if (dtGridReflectionObj.sortParameter && dtGridReflectionObj.sortParameter.columnId 
								&& dtGridReflectionObj.sortParameter.columnId.length>0 && dtGridReflectionObj.sortParameter.sortType!=0) {
							params.sortParameter = JSON.stringify(dtGridReflectionObj.sortParameter);
						}
						...
						$.ajax({
							...
							success:function(datas){
								...
								var pager = {};
								if (dtGridReflectionObj.option.didLoadedDataPreprocess) {
									var preResult = dtGridReflectionObj.option.didLoadedDataPreprocess('pager', datas);
									if (typeof preResult=='object') {
										pager = preResult;
										dtGridReflectionObj.pager = pager; //用服务端返回的pager更新客户端pager
									}
								} else {
									pager = json_parse(datas);//$.parseJSON(datas);
								}
								//===modify by nick end===
				...
				constructGrid : function(){
					...
					if(dtGridReflectionObj.exhibitDatas!=null){
						if(dtGridReflectionObj.option.sortByRemote==false && dtGridReflectionObj.sortParameter&&dtGridReflectionObj.sortParameter.columnId){
					...
					$('#dt_grid_'+gridId+' .can-sort').click(function(){
						...
						//重新加载数据
						if (dtGridReflectionObj.option.sortByRemote==false) //本地缓存数据排序
							dtGridReflectionObj.reload();
						else if (dtGridReflectionObj.option.sortByRemote==true) //远程服务端查询排序
				...
				constructGridPageBar : function(){
					...
					//触发工具栏创建完成事件 - by nick
					if (dtGridReflectionObj.option.onPageBarComplete) {
						dtGridReflectionObj.option.onPageBarComplete(dtGridReflectionObj);
					}
				},
				//工具条基础内容
				...
				showProcessBar : function(){
					...
					// content += '<div class="dt-grid-process-bar-bottom bg-primary" id="dt_grid_process_bar_bottom_'+dtGridReflectionObj.option.id+'"></div>'; 废除下部进度条
				...
				hideProcessBar : function(callback){
					...
					clearInterval(dtGridReflectionObj.processBarThread);
					var dtGridReflectionObj = this;
					$('#dt_grid_process_bar_top_'+dtGridReflectionObj.option.id).animate({width:'97%'/*'100%'*/}, 100, function(){ //改为97%防止出现滚动条
						$('#dt_grid_process_bar_top_'+dtGridReflectionObj.option.id).remove();
						if (callback)
							callback();
					});
					$('#dt_grid_process_bar_bottom_'+dtGridReflectionObj.option.id).animate({width:'97%'/*'100%'*/}, 100, function(){ //改为97%防止出现滚动条
						$('#dt_grid_process_bar_bottom_'+dtGridReflectionObj.option.id).remove();
						//if (callback)
							//callback();
					});
				...
				formatContent:function(column, value){
					var dtGridReflectionObj = this; //by nick
					try{
						//如果是codeTable
						if(column.codeTable){
							var codeTableValue = column.codeTable[value];
							return codeTableValue?codeTableValue:(dtGridReflectionObj.option.defaultCodeTableValue?dtGridReflectionObj.option.defaultCodeTableValue:value);
						}
						...
						if(column.type=='date'&&column.format){
							if(column.otype){
								...
								if (typeof value=='string') //支持字符串类型的date格式化 by nick
									return $.fn.DtGrid.tools.dateFormat(new Date(value.replace(/-/g,"/")), column.format);
								else 
									return $.fn.DtGrid.tools.dateFormat(value, column.format);
							}
						}
...
//					});
				...
				reload : function(allReload){
					//定义表格对象映像
					var dtGridReflectionObj = this;
					dtGridReflectionObj.option.stopEvent=0; //解除事件传递状态 modify by nick
					if(allReload){
						dtGridReflectionObj.load();
					}else{
						//===触发即将刷新视图事件 by nick===
...
(function($) {
	$.fn.DtGrid.defaultOptions = {
		grid : {
			...
			sortByRemote : false,	//false=本地缓存数据排序，true=远程服务端查询排序
			stopEvent: 0, 	//0=解除阻止事件传递，1=阻止事件传递
			defaultCodeTableValue: null, //代码对照表对应值不存在时的显示方式：null=显示代码；其它=显示默认值
			pageSize : 20,
